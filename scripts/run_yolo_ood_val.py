...
from pig_pipeline.training.yolo import load_yolo_model, _predict_batch, flush_inference_memory
...
def _save_confusion_matrix(y_true: list[int], y_pred: list[int], class_names: list[str], out_path: Path) -> None:
...
def main() -> None:
	...
	with metrics_path.open("w", encoding="utf-8") as f:
		json.dump(
			{"top1": metrics["top1"], "macro_f1": metrics["macro_f1"], "report": metrics["report"]},
			f,
			indent=2,
		)
